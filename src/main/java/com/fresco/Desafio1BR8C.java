	...
	public static void procesar() throws FileNotFoundException, IOException {
		...
		try (var fileMeasurements = new RandomAccessFile(new File(FILE), "r")) {
			...
			while (offset < length) {
				...
				chunk++;
				workers.add(new Worker(byteBuffer, chunk));
				offset += chunk;
			}
	...
	static class Worker {
		...
		public void execute() {
			try {
				...
				while (count < size) {
					...
					var medicion = mapMediciones.get(estacion.flip());
					if (medicion == null) {
						var newEstacion = ByteBuffer.allocate(50).put(estacion).flip();
						medicion = mapMediciones.computeIfAbsent(newEstacion, e -> new IntSummaryStatistics());
					}
		...
		public Worker combine(Worker o) {
			for (var entry : o.mapMediciones.entrySet()) {
				mapMediciones.merge(entry.getKey(), entry.getValue(), (a, b) -> {
					a.combine(b);
					return a;
				});
			}
