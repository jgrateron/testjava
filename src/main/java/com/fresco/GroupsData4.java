...
public class GroupsData4 {
	...
	public static void main(String[] args) {
		...
		Comparator<Map.Entry<String, List<WorldCup>>> comparar = (e1, e2) -> {
		...
		mapMundiales.entrySet().stream()//
				...
				.forEach(e -> {
					var years = e.getValue().stream().map(WorldCup::year).toList();
					System.out.println("%-10s: %d %s".formatted(e.getKey(), e.getValue().size(), years));
				});
		System.out.println("-".repeat(50));
	}
}
